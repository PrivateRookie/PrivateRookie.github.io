...
fn main() {
    ...
    match cmd {
                ...
                'out: loop {
                    ...
                    loop {
                        if let Ok(msg) = state.consume() {
                            ...
                            if write_buf.len() >= BUF_SIZE || msg.len == 0 {
        ...
        Command::C {
            ...
            for i in 0..(size / batch + 1) {
                let send_size = if batch * (i + 1) > size {
                    (size - batch * (i + 1)) as usize
                } else {
                    batch as usize
                };
                if let Err(e) = stream.write_all(&buf.repeat(send_size)) {
                    tracing::error!("{e}");
                    break;
                }
                info!("batch send done. wait for echo");
                let mut echoed = 0;
                'out: loop {
                    ...
                    while let Ok(_) = state.consume() {
                        echoed += 1;
                        if echoed == send_size {
                            info!("all echo message received");
                            break 'out;
                        }
                    }
                }
